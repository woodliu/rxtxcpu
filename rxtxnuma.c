...
static const struct option long_options[] = {
  ...
  {"numa-list",       required_argument, NULL, 'l'},
  {"numa-mask",       required_argument, NULL, 'm'},
...
static const struct usage_opt usage_options[] = {
  ...
  {'d', "DIRECTION", "Capture only packets matching DIRECTION. DIRECTION can"
                        " be 'rx', 'tx', or 'rxtx'. Default matches invocation"
                         " (i.e. DIRECTION defaults to 'rx' when invocation is"
                               " 'rxnuma', 'tx' when 'txnuma', and 'rxtx' when"
                                                              " 'rxtxnuma')."},
  {'h', NULL,        "Display this help and exit."},
  {'l', "NUMALIST",  "Capture only on numa nodes in NUMALIST (e.g. if NUMALIST"
                        " is '0,2-4,6', only packets on numa nodes 0, 2, 3, 4,"
                                                  " and 6 will be captured)."},
  {'m', "NUMAMASK",  "Capture only on numa nodes in NUMAMASK (e.g. if NUMAMASK"
                       " is '5d', only packets on numa nodes 0, 2, 3, 4, and 6"
                                                        " will be captured)."},
  ...
  {'w', "FILE",      "Write packets to FILE in pcap format. FILE is used as a"
                     " template for per-numa-node filenames (e.g. if capturing"
                      " on a system with 2 numa nodes, numa nodes 0 and 1, and"
                     " FILE set to 'out.pcap', the numa node 0 capture will be"
                         " written to 'out-0.pcap' and the numa node 1 capture"
                      " will be written to 'out-1.pcap'). Writing to stdout is"
                             " supported by setting FILE to '-', but only when"
                                          " capturing on a single numa node."},
  {0, NULL, NULL}
};
...
int main(int argc, char **argv) {
   ...
   */
  if (strcmp(program_basename, "rxnuma") == 0) {
    status = rxtx_set_direction(&rtd, PCAP_D_IN);
  } else if (strcmp(program_basename, "txnuma") == 0) {
    ...
    switch (c) {
      ...
      case 'l':
        ...
        if (parse_cpu_list(optarg, &ring_set)) {
          fprintf(stderr, "%s: Invalid numa list '%s'.\n", program_basename,
                                                                       optarg);
          usage_short();
          return EXIT_FAIL_OPTION;
        }
      ...
      case 'm':
        ...
        if (parse_cpu_mask(optarg, &ring_set)) {
          fprintf(stderr, "%s: Invalid numa mask '%s'.\n", program_basename,
                                                                       optarg);
          usage_short();
          return EXIT_FAIL_OPTION;
        }
  ...
  if (list && mask) {
    fprintf(stderr, "%s: -l [--numa-list] and -m [--numa-mask] are mutually"
                                            " exclusive.\n", program_basename);
    usage_short();
    return EXIT_FAIL_OPTION;
  }
  ...
  rings = num_numa();
  if (rings <= 0) {
    fprintf(stderr, "%s: Failed to get numa node count.\n", program_basename);
    return EXIT_FAIL;
  }
  ...
  if (rxtx_verbose_isset(&rtd)) {
    fprintf(stderr, "Found '%d' numa nodes.\n", rxtx_get_ring_count(&rtd));
  }
  ...
  if (!worker_count) {
    fprintf(stderr, "%s: No configured numa nodes present in numa %s.\n",
  ...
  if (get_online_numa_set(&online) != 0) {
    fprintf(stderr, "%s: Failed to get online numa set.\n", program_basename);
    return EXIT_FAIL_OPTION;
  }
  ...
  if (CPU_COUNT(&online) != rxtx_get_ring_count(&rtd)) {
    for_each_ring(i, &rtd) {
      if (!CPU_ISSET(i, &online)) {
        RING_CLR(i, &ring_set);
        if (rxtx_verbose_isset(&rtd)) {
          fprintf(stderr, "Skipping numa node '%d' since it is offline.\n", i);
        }
      }
    }
  }
  ...
  if (!worker_count) {
    fprintf(stderr, "%s: No online numa nodes present in numa %s.\n",
                                 program_basename, list ? "list" : "mask");
    usage_short();
    return EXIT_FAIL_OPTION;
  }
  ...
  for_each_set_ring(i, &rtd) {
    ...
    fprintf(out, "%ju packets captured on numa node %d.\n",
